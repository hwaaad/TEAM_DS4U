...
<body>
	...
	<input type="hidden" value="board" id="pageType">
		<div id="wsBodyContainer">
			...
			<%
				if (apvDAO.apvSearchCount(searchType, search, PageNumber) <= 0) {
			%>
				<table class="table" style="text-align: center; border: 1px solid #dddddd">
						<tbody>
							<tr>
								<td style="width: 1690px; text-align: center; border-right:1px solid #dddddd">검색 결과가 없습니다.</td>
							</tr>
							<tr>
								<td colspan="9">
			...
			<%
				...
				//ArrayList<ApvDTO> searchList = new ArrayList<ApvDTO>();
				//searchList = new ApvDAO().getSearch(searchType, search, PageNumber);
				//for (int i=0; i<searchList.size(); i++) {
				//	if (i == 10) break;
				//	ApvDTO apv = apvList.get(i);
				for (int i=0; i<apvList.size();i++) {
					ApvDTO apv = apvList.get(i);
					if(searchType.equals("사업명") && !apv.getAPV_NM().contains(search))
						continue;
				...
				<tr>
					<td colspan="9">
						...
						<div id="searchWrap">
								<form action="./apvSearch.jsp" method="get" id="boardSearchForm">							
									<select name="searchType" id="searchType">
										<option value="사업명" <% if (searchType.equals("사업명")) out.println("selected"); %>>사업명</option>
										<option value="사업담당자" <% if (searchType.equals("사업담당자")) out.println("selected"); %>>사업담당자</option>
									</select>
									<input type="text" name="search" type="submit"><button>검색</button>
								</form>
